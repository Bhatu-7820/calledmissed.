...
export async function POST(req: NextRequest) {
  try {
    const { messages, prompt, image, temperature, systemPrompt, customApiKey } = await req.json();
    ...
    // Format the messages list for vision
    let payloadMessages = [];

    if (messages && messages.length > 0) {
      // If we are passing full message history, append the new image message at the end
      ...
      return new Response(
        JSON.stringify({ error: "Vision endpoint requires at least an image and a prompt.", code: "bad_request" }),
